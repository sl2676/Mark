...
export function AnimatedTabBarView(props) {
  // props
  const {
  ...
  if (!Object.keys(Presets).includes(preset)) {
    throw new Error(`Wrong preset been provided. expected one of these: [${Object.keys(Presets).join(', ')}], but found "${preset}".`);
  } // variables
  ...
  const animatedOnChange = useMemo(() => proc(index => call([index], args => {
    if (onIndexChange) {
  ...
  const PresetComponent = Presets[preset].component; // render

  return /*#__PURE__*/React.createElement(PresetComponent, _extends({
    style: style,
